...
pub struct Config {
    ...
    /// This instance's name in a swarm mailbox (see `tools::swarm`). `None`
    /// means the Swarm* tools are unconfigured and refuse to run — there is
    /// no default name, since a wrong guess would let one agent silently
    /// answer for another's inbox.
    pub swarm_agent_name: Option<String>,
}
...
impl Config {
            ...
            if let Some(eq_pos) = line.find('=') {
                ...
                if let Some(ref mut p) = current_provider {
                    ...
                    match key {
                        ...
                        "swarm_agent_name" => {
                            if !value.is_empty() {
                                config.swarm_agent_name = Some(String::from(value));
                            }
                        }
                        _ => {}
                    }
    ...
    fn serialize(&self) -> String {
        ...
        if let Some(ref name) = self.swarm_agent_name {
            content.push_str("swarm_agent_name=");
